...
<FontAwesomeIcon icon={["fas", "heartbeat"]} size="3x" color="#4285f4" />
...
The other team members are then allowed to react.
Either with a friendly
...
Of course, the weaknesses no one has seen in themselves are interesting.
Keep them for the next training session.
